...
class DummyAgent(CaptureAgent):
  ...
  def getReward(self, currentState, lastState):
    ...
    reward += 5*(len(lastfoodList) - len(currentfoodList))

    # if len(currentfoodList) < len(lastfoodList) and self.index == 0:
    #   print("Eaten food: food count: %d" % len(currentfoodList))
    # elif len(currentfoodList) > len(lastfoodList) and self.index == 0:
    ...
    if self.getMazeDistance(currentState.getAgentState(self.index).getPosition(), lastState.getAgentState(self.index).getPosition()) > 3:
      reward -= 30
      # if self.index == 0:
        # print("Got eaten")
...
class OffensiveAgent(DummyAgent):
    ...
    def getFeatures(self, gameState, action):
        ...
        if len(ghosts) > 0:
            ...
            if successor.getAgentState(self.index).isPacman and min(dists) <= 5:
                features['ghostDistnace'] = min(dists)

        # Number of legal actions
        features["legalActions"] = len(gameState.getLegalActions())

        scaredGhosts = [a for a in enemies if not a.isPacman and a.scaredTimer != 0 and a.getPosition() != None]
        if len(scaredGhosts) > 0:
            dists = [self.getMazeDistance(myPos, a.getPosition()) for a in scaredGhosts]
            if successor.getAgentState(self.index).isPacman:
                features['scaredGhostDistance'] = min(dists)
                scaredGhostsLastPos = self.getScaredGhostsPos(gameState)
                if self.isEnemyEaten(myPos, scaredGhostsLastPos):
    ...
    def getScaredGhostsPos(self, gameState):
        enemies = [gameState.getAgentState(i) for i in self.getOpponents(gameState)]
        scaredGhosts = [a for a in enemies if not a.isPacman and a.scaredTimer != 0 and a.getPosition() != None]
        return [i.getPosition() for i in scaredGhosts]


    def isEnemyEaten(self, myCurrentPos, enemiesLastPos):
        for enemyPos in enemiesLastPos:
            if enemyPos == myCurrentPos:
                return True
        return False
    ...
    def getFeatures(self, gameState, action):
        ...
        if len(invaders) > 0:
            ...
            if agentState.scaredTimer != 0:# and not agentState.isPacman:# and min(dists) < 5:
                features['invaderAlert'] = min(dists)
